      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-1 flex flex-col items-center p-6 text-center">
          <Avatar className="w-32 h-32 mb-4">
            <AvatarImage src="https://i.pravatar.cc/150?u=ahmad" alt={teacher.name} />
            <AvatarFallback>AH</AvatarFallback>
          </Avatar>
          <h3 className="text-xl font-semibold">{teacher.name}</h3>
          <p className="text-gray-500 mb-2">{teacher.nip}</p>
          <Badge variant="secondary" className="mb-6 bg-indigo-100 text-indigo-800 hover:bg-indigo-100">Guru {teacher.subject}</Badge>
